...
public class EndtestClient {
    ...
    public String startExecution(String apiRequest, String appId, String appCode)
            throws IOException, InterruptedException {
        URI uri = buildAuthenticatedRequest(apiRequest, appId, appCode);
        String responseBody = sendGet(uri);
        String hash = cleanResponse(responseBody);

        if (hash.isEmpty()) {
            throw new IOException("Endtest returned an empty execution hash.");
        }

        if (looksLikeHtml(hash)) {
            throw new IOException("Endtest returned an HTML page instead of an execution hash.");
        }

        return hash;
    }

    public ResultResponse getResults(String appId, String appCode, String hash)
            throws IOException, InterruptedException {
        ...
        if ("Erred.".equals(responseBody)) {
            throw new IOException("The Endtest execution ended with status: Erred.");
        }
        ...
        result.put("detailedLogs", readString(json, "detailed_logs"));
        result.put("screenshotsAndVideo", readString(json, "screenshots_and_video"));
        result.put("hash", hash);
        result.put("resultsUrl", "https://app.endtest.io/results?hash=" + hash);

        return ResultResponse.completed(result);
    }
    ...
    private URI buildAuthenticatedRequest(String apiRequest, String appId, String appCode) {
        ...
        if (hasQueryParameter(uri, "appId") || hasQueryParameter(uri, "appCode")) {
            throw new IllegalArgumentException(
                    "Remove appId and appCode from apiRequest. " + "They are supplied through Jenkins Credentials.");
        }
        ...
        return URI.create(request + separator + "appId=" + encode(appId) + "&appCode=" + encode(appCode));
    }
